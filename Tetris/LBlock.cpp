...
LBlock::LBlock() {
	...
	blocks[2].setPosition(Vector2f(6 * BRICK_SIZE, 0));
	blocks[3].setPosition(Vector2f(6 * BRICK_SIZE, -BRICK_SIZE));
}

void LBlock::rotateBlock(Field f) {
	...
	case 0:
		...
		if (position.y == 570 && field[x - 2][y] == 'x') return;
	...
	case 2:
		...
		if (field[x - 1][y] == 'x' || field[x - 1][y - 1] == 'x' || field[x + 1][y] == 'x') return;
	...
	case 3:
		...
		// Jesli wyszedlem za ekran, ale nie moge obrocic
		if (position.x == 270 && field[x][y - 2] == 'x') return;
